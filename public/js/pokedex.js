...
let partnerSpotlight = null;
  ...
  async function loadSubcategories() {
    try {
      ...
      subcategories = res.ok ? await res.json() : [];
    } catch {
      subcategories = [];
    }
  }

  async function loadPartnerSpotlight() {
    try {
      const res = await fetch("/api/partner");
      partnerSpotlight = res.ok ? await res.json() : null;
    } catch {
      partnerSpotlight = null;
    }
  }

  function getSubcategoryLabel(card) {
    return card?.subcategory_label || card?.subcategory?.label || null;
  }


  function extraText(card) {
    const badges = [];
    const subLabel = getSubcategoryLabel(card);
    if (subLabel) badges.push(`<span class="badge text-bg-primary">${escapeHtml(subLabel)}</span>`);
    if (norm(card.type) === "weed" && card.weed_kind) {
      badges.push(`<span class="badge text-bg-success">${escapeHtml(card.weed_kind)}</span>`);
    }
    if (norm(card.type) !== "weed" && card.micron) {
      badges.push(`<span class="badge text-bg-warning">${escapeHtml(card.micron)}</span>`);
    }
    return badges.join(" ");
  }
  ...
  function renderPartnerSpotlight() {
    ...
    const title = partnerSpotlight.title || "🤝 Partenaire du moment";
    const desc = partnerSpotlight.description || "";
    const url = partnerSpotlight.url || "";
    const img = partnerSpotlight.image_url || "";

    partnerBox.innerHTML = `
      <div class="featuredCard">
        ${img ? `<img class="featuredImg" src="${img}" alt="Partner" />` : ""}
        <div class="featuredBody">
          <div class="featuredTitle">${escapeHtml(title)}</div>
          ${desc ? `<div class="featuredDesc">${escapeHtml(desc)}</div>` : ""}
          ${url ? `<a class="btn btn-sm btn-primary mt-2" href="${url}" target="_blank" rel="noreferrer">Voir</a>` : ""}
        </div>
      </div>
    `;
  }
  ...
  function renderSubChips() {
    ...
    const makeChip = (label, value) =>
      `<button class="chip ${activeSub === value ? "active" : ""}" data-sub="${value}">${label}</button>`;

    chips.push(makeChip("All", "all"));

    // Legacy chips (micron / weed_kind)
    if (activeType === "weed") {
      ["indica", "sativa", "hybrid"].forEach((k) => chips.push(makeChip(k, k)));
    } else {
      ["120u", "90u", "73u", "45u"].forEach((m) => chips.push(makeChip(m, m)));
    }

    // DB subcategories
    const list = (subcategories || []).filter((s) => norm(s.type) === activeType);
    if (list.length) {
      chips.push(`<span class="chip-sep">•</span>`);
      list.slice(0, 40).forEach((s) => chips.push(makeChip(s.label, `subcat:${s.id}`)));
    }
